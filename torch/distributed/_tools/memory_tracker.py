...
class MemoryProfileDispatchMode(TorchDispatchMode):
    ...
    def __torch_dispatch__(self, func, types, args=..., kwargs=None):
        ...
        if func == torch.ops.aten.detach.default:
            return rs
        func_name: str = (
            self.memory_tracker.cur_module
            + "."
            + func.__name__
            + "_"
            + str(self.memory_tracker.operator_names[func.__name__])
        )
        self.memory_tracker.operator_names[func.__name__] = (
            self.memory_tracker.operator_names[func.__name__] + 1
        )
...
class MemoryTracker:
    ...
    def __init__(self) -> None:
        ...
        self.operator_names: Dict[str, int] = defaultdict(int)
        self.memories_allocated: Dict[str, float] = defaultdict(float)
        self.memories_active: Dict[str, float] = defaultdict(float)
        self.memories_reserved: Dict[str, float] = defaultdict(float)
        self.markers: Dict[str, int] = defaultdict(int)
        self.cur_module: str = ""
        self.op_id: int = 0
    ...
    def start_monitor(self, root_module: nn.Module) -> None:
        ...
        module level and operator level memory stats can be tracked during module
        runtime.
        """
        self._clear_state()
        root_module.__setattr__("is_root", True)
        for name, m in root_module.named_modules():
            if m is not root_module:
                m.__setattr__("is_root", False)
    ...
    def stop(self) -> None:
        ...
        tracking memory stats at module level and operator level.
        """
    ...
    def _create_pre_forward_hook(self, name: str) -> Callable:
        ...
        def _pre_forward_hook(module: nn.Module, inputs: Any) -> None:
            self.cur_module = f"{name}.forward"
            if hasattr(module, "is_root") and module.is_root:
    ...
    def _create_backward_hook(self, name: str) -> Callable:
        ...
        def _backward_hook(
            ...
            self.cur_module = f"{name}.backward"
    ...
    def _record_memory_stats(self, fn_name: str) -> None:
        """
        Record current allocated memory, current active memory and current reserved memory,
        the memory stats dict is indexed with 'self.op_id'.
        """
        memory_allocated: float = torch.cuda.memory_allocated() / MB
        memory_reserved: float = torch.cuda.memory_reserved() / MB
        memory_active: float = (
            torch.cuda.memory_stats().get("active_bytes.all.current", 0) / MB
        )
        self.memories_allocated[self.op_id] = (fn_name, memory_allocated)
        self.memories_reserved[self.op_id] = (fn_name, memory_reserved)
        self.memories_active[self.op_id] = (fn_name, memory_active)
        self.op_id += 1

    def _add_marker(self, marker_name: str) -> None:
        """
        Get the x axis's value of the marker.
        """
